...
class BBSignalGeneratorTestCase(TestCase):
    ...
    def test_generate_signals(self):
        ...
        self.assertEquals(BBSignalGenerator(indicator=BollingerIndicator(df=self.df, time_period=5, dimension="close",
                                                                         sigma=1)).generate_signals().equals(
            self.signals_df), True)
